...
class AuthService:
    @staticmethod
    async def save_cookies(context: BrowserContext, path: str = "cookies.json") -> None:
        """쿠키를 파일로 저장"""
        try:
    ...
    @staticmethod
    async def load_cookies(context: BrowserContext, path: str = "cookies.json") -> bool:
        """파일에서 쿠키 불러오기"""
    ...
    @classmethod
    async def login(cls, username: str, password: str) -> Dict[str, str]:
        """
        ...
        try:
            async with async_playwright() as p:
                ...
                # 2. 쿠키 로드 및 로그인 상태 확인
                if await cls.load_cookies(context):
                ...
                if found_cookie:
                    ...
                    await cls.save_cookies(context)
                    return cookies_dict
                else:
