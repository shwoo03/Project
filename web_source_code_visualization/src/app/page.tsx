...
export default function Dashboard() {
  ...
  /* 
   * Opens the Campaign Runner in a new separate window
   * as requested by user ("밑에 모달 띄우지 말고 따로 창을 새로 띄워줘")
   */
  const handleAutoPwn = () => {
    // Save data for the new window to pick up
    sessionStorage.setItem('campaignData', JSON.stringify({ routes, targetPath }));
    ...
    window.open('/campaign', 'AutoPwnMission', `width=${width},height=${height},left=${left},top=${top}`);
  };

  /* 
  * Analyzes the target directory
  */
    ...
    <>
      <div className="flex h-screen w-full bg-slate-950 text-slate-100 overflow-hidden font-sans">
                        ...
                        <div key={idx} className="bg-red-950/40 p-3 rounded border border-red-900/60">
                          ...
                          {sink.flowPath && sink.flowPath.length > 0 && (
                            <div className="mt-2 pl-2 border-l-2 border-red-900/50 space-y-1">
                              ...
                              {sink.flowPath.map((step: any, stepIdx: number) => (
        ...
        <div className="flex-1 flex flex-col h-full">
          ...
          <header className="h-16 border-b border-slate-800 flex items-center px-6 justify-between bg-slate-950/50 backdrop-blur z-20">
            ...
            <div className="ml-2 flex items-center gap-2">
              ...
              <button
                ...
                className="bg-purple-600 hover:bg-purple-500 text-white text-[10px] h-9 px-3 rounded-lg font-bold flex items-center gap-1 disabled:opacity-50 disabled:grayscale transition-all"
              >
                <Crosshair size={12} /> AUTO-PWN
              </button>
            </div>
          ...
          {/* Campaign Modal Removed - Now runs in separate window */}
      ...
      {/* Modal removed. Now opens in new window via handleAutoPwn */}
      <ExploitTerminal />
    </>
  );
}
